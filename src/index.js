...
async function sendPrizeConfirmation(env, prize, subject, message) {
  ...
  // Eén kader met de prijs, de omschrijving (websitetekst) en de voorwaarden.
  const descBlock = prize.description ? `
            <div style="font-size:14px;line-height:1.55;color:#3a3320;margin-top:8px;">${nl2br(prize.description)}</div>` : "";
  const condBlock = prize.conditions ? `
            <div style="border-top:1px solid #f1d79a;margin-top:12px;padding-top:10px;">
              <div style="font-size:12px;font-weight:bold;color:#9a7b22;">De prijs is ingebracht onder de volgende voorwaarden:</div>
              <div style="font-size:14px;line-height:1.55;color:#3a3320;margin-top:3px;">${nl2br(prize.conditions)}</div>
            </div>` : "";
  const prizeBox = `
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:separate;margin:8px 0 18px;">
          <tr><td style="background:#fff8e9;border:2px dashed #f7a81b;border-radius:14px;padding:16px 20px;font-family:Arial,Helvetica,sans-serif;">
            ...
            <div style="font-family:Georgia,'Times New Roman',serif;font-size:21px;font-weight:bold;color:#17458f;line-height:1.2;margin-top:2px;">🎁 ${escHtml(prize.title)}${prize.value ? ` <span style="font-size:14px;color:#9a7b22;font-weight:normal;">${escHtml(prize.value)}</span>` : ""}</div>${descBlock}${condBlock}
          </td></tr>
        </table>`;
  // Plaats het kader op de {{prijs}}-marker in het bericht; geen marker -> onderaan het bericht.
  const parts = String(message).split(/\{\{\s*prijs\s*\}\}/i);
  const bodyHtml = parts.length > 1
    ? parts.map((p) => (p.trim() ? htmlParagraphs(p) : "")).join(prizeBox)
    : htmlParagraphs(message) + prizeBox;
  const html = `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#eef3fb;margin:0;padding:0;">
  <tr><td align="center" style="padding:24px 12px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background:#ffffff;border-radius:18px;overflow:hidden;">
      ...
      <tr><td style="padding:28px 34px 18px;font-family:Arial,Helvetica,sans-serif;color:#1d2433;font-size:16px;line-height:1.6;">
        ${bodyHtml}
      </td></tr>
      <tr><td style="background:#0e2d63;padding:26px 34px 28px;font-family:Arial,Helvetica,sans-serif;">
        <img src="https://nijmegenduckstad.nl/assets/img/rotary-nijmegen-stadenland.png" alt="Rotary Nijmegen Stad en Land" width="210" style="display:block;width:210px;max-width:210px;height:auto;background:#ffffff;border-radius:8px;padding:9px;">
